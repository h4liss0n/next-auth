...
export const ProjectTask: React.FC<Props> = ({ projectId, tasks, onDelete, onStatus }) => {


    if (tasks.length === 0) {
        return (
            <div className="flex-1 p-2">
                <p className="text-sm font-semibold leading-6 text-gray-900">No tasks</p>
            </div>
        )
    }
                ...
                {tasks.map(task => (
                    <li key={task.id}>
                        <div className="flex items-center justify-center">
                            ...
                            <div className="p-2">
                                <input type="checkbox" name="done" checked={task.done} onChange={() => onStatus(task.id, !task.done)} />
                            </div>
                            <div className="p-2">
